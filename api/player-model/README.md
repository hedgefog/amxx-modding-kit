...
The **Player Model API** provides advanced tools for managing player models and animations in GoldSrc-based games. It supports custom model switching, animation files, and automatic animation extension handling.

## 🚀 Features

- **Dynamic Model Management**: Set, update, and reset player models at runtime
- **Custom Animations**: Load custom animations from separate files
- **Automatic Animation Switching**: Seamlessly switch animation models based on weapon animation extensions
- **Sequence Control**: Set specific animation sequences for players

## 📚 Usage

### 🧍 Setting and Updating Player Models

Use `PlayerModel_Set` to assign a model to a player, and `PlayerModel_Update` to apply the changes.

```pawn
#include <api_player_model>

public plugin_init() {
    register_plugin("Player Model Example", "1.0", "Author");

    register_clcmd("say /model", "Command_SetModel");
}

public Command_SetModel(const pPlayer) {
    PlayerModel_Set(pPlayer, "models/mymod/custom_model.mdl");
    PlayerModel_Update(pPlayer);

    client_print(pPlayer, print_chat, "Your player model has been updated!");

    return PLUGIN_HANDLED;
}
```

### 🔄 Resetting to Default Models

Use `PlayerModel_Reset` to revert a player’s model to the default.
...
Verify if a player has a custom model using `PlayerModel_HasCustom`.

```pawn
if (PlayerModel_HasCustom(pPlayer)) {
    client_print(pPlayer, print_chat, "You are using a custom model.");
}
```
...
### Precaching Animations

Use `PlayerModel_PrecacheAnimation` to precache an animation file from the `animations/` directory. This example precaches `cstrike/animations/mymod/player.mdl`:

```pawn
PlayerModel_PrecacheAnimation("mymod/player.mdl");
```

### Setting Custom Weapon Animation

Switch animation models automatically by updating the player’s animation extention.

```pawn
set_ent_data_string(pPlayer, "CBasePlayer", "m_szAnimExtention", "myweapon");
```

#### Setting Specific Animation Sequences

Use `PlayerModel_SetSequence` to set specific animation sequences for a player.


Customize weapon animations by setting the `m_szAnimExtention` member:
...
The API will automatically select the correct animation model file for the current animation extension.

### Setting Specific Sequences
...
Your animation model must provide standard player sequences like `walk`, `run`, `flinch`, etc.
...
Your animation model must include at least one polygon. Use a `$body` command in your QC file to add the reference:

```
$body "studio" "animreference"
```

The `animreference.smd` file should contain a minimal skeleton with one triangle. See the standard Counter-Strike player model skeleton for the bone structure.

### Protecting Bones

Use a compiler like `DoomMusic's StudioMDL` to protect bones. Add `$protection` for each bone before references and sequences in the `.qc` file:
...
### Custom Weapon Animation Sequences

Add your custom weapon animations using blended sequences:
...
See [`api_player_model.inc`](include/api_player_model.inc) for all available natives.
